...
function MobileMenu() {
    ...
    return (
        <div className={`${!showMenu && 'hidden' } ${menu === 1 || menu===5 ? 'h-[calc(100vh-5rem)] ' : ''} lg:hidden  bg-[#5C896E] text-white fixed z-50 rounded-t-3xl bottom-0  w-full flex-shrink-0`}>
            <div className="flex justify-center">
                
            </div>
            <div className="flex h-12  justify-between items-center px-10">
                <div className=""></div>
                <div className='h-2 rounded-full w-28 bg-white' />
                <button onClick={() => dispatch(setShowMenu(false))}><Close className='text-white' /></button>
            </div>
