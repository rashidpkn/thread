    ...
    <div className='space-y-5 relative h-full'>
        ...
        <p className='text-center font-medium'>What are you measuring for?</p>
          <div className="flex justify-center gap-5">
            <div onClick={() => { dispatch(setItem('Curtain')) }} className={`w-24 h-24 rounded-md`}><img className='w-full' src={item === 'Curtain' ? Flat : dFlat} alt="" /></div>
            <div onClick={() => { dispatch(setItem('Blind')) }} className={`w-24 h-24 rounded-md`}><img className='w-full' src={item === 'Blind' ? OutSide : dOutSide} alt="" /></div>
          </div>
          {
            item === 'Curtain' && <div className="space-y-3">
              <p className='text-center font-medium'>What kind of window to you have?</p>
              <div className="flex justify-center gap-5">
                <div onClick={()=>{dispatch(setInstallation('Curtain Flat'))}}  className={`w-24 h-24 rounded-md`}> <img className='w-full' src={installation === 'Curtain Flat' ? Flat : dFlat} alt="" /> </div>
                <div onClick={()=>{dispatch(setInstallation('Curtain Bay'))}} className={`w-24 h-24 rounded-md`}> <img className='w-full' src={installation === 'Curtain Bay' ? Bay : dBay} alt="" /> </div>
              </div>
            </div>
          }

          {
            item === 'Blind' && <div className="space-y-3">
              ...
              <div className="flex justify-center gap-5">
                <div onClick={()=>{dispatch(setInstallation('Blind Inside'))}} className={`w-24 h-24 rounded-md`}> <img className='w-full' src={installation === 'Blind Inside' ? Inside :dInside} alt="" /> </div>
                <div onClick={()=>{dispatch(setInstallation('Blind OutSide'))}} className={`w-24 h-24 rounded-md`}> <img className='w-full' src={installation === 'Blind OutSide' ? OutSide : dOutSide} alt="" /> </div>
              </div>
            </div>
          }
        ...
        <div className="absolute bottom-5 w-full h-12 flex justify-between items-center px-10">
            ...
            <div className="cursor-pointer h-10 w-32 bg-[#b68d40] flex justify-center items-center rounded-xl" onClick={()=>{setSelector(2)}}>Next</div>
        </div>
    </div>
  )
}
