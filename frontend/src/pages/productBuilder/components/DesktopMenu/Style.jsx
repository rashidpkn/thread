  ...
  return (
    <div className='space-y-5'>
      ...
      <div className="flex justify-center gap-5 flex-wrap">
        ...
        <div onClick={() => { dispatch(setLook('Eyelet')); dispatch(setPrice()); }} className={` w-24 h-24 rounded-md flex justify-center items-center relative`} ><img className='w-full' src={look === 'Eyelet' ? Eyelet : dEyelet} alt="" />
          <p className='absolute -bottom-6 text-center'>Eyelet</p>
          <InfoOutlined onClick={()=>dispatch(setShow3D(true))}  fontSize='small' className='absolute top-1 right-1 bg-blue-500 text-white rounded-full' aria-label="Let's try out 3D" titleAccess="Let's try out 3D" />
        </div>
        <div onClick={() => { dispatch(setLook('Pencil')); dispatch(setPrice()); }} className={` w-24 h-24 rounded-md flex justify-center items-center relative`} ><img className='w-full' src={look === 'Pencil' ? Pencil : dPencil} alt="" />
          <p className='absolute -bottom-6 text-center'>Pencil</p>
          <InfoOutlined onClick={()=>dispatch(setShow3D(true))}  fontSize='small' className='absolute top-1 right-1 bg-blue-500 text-white rounded-full' aria-label="Let's try out 3D" titleAccess="Let's try out 3D" />
        </div>
        <div onClick={() => { dispatch(setLook('Goblet')); dispatch(setPrice()); }} className={` w-24 h-24 rounded-md flex justify-center items-center relative`} ><img className='w-full' src={look === 'Goblet' ? Goblet : dGoblet} alt="" />
          <p className='absolute -bottom-6 text-center'>Goblet</p>
          <InfoOutlined onClick={()=>dispatch(setShow3D(true))}  fontSize='small' className='absolute top-1 right-1 bg-blue-500 text-white rounded-full' aria-label="Let's try out 3D" titleAccess="Let's try out 3D" />
        </div>
        <div onClick={() => { dispatch(setLook('Triple')); dispatch(setPrice()); }} className={` w-24 h-24 rounded-md flex justify-center items-center relative`} ><img className='w-full' src={look === 'Triple' ? Triple : dTriple} alt="" />
